    ...
    <div class="page">
      ...
      <brew-card title="Brew Parameters" class="detail-card">
        <div class="params-display">
          ...
          @if (brew()!.waterTemp) {
          <div class="param-item">
            <span class="param-value">{{ brew()!.waterTemp }}°</span>
            <span class="param-label">temp (C)</span>
          </div>
          } @if (brew()!.brewTimeSeconds) {
          <div class="param-item">
        ...
        <div class="grind-info">
          ...
          {{ brew()!.grindDescription }}
        </div>
        }
      </brew-card>

      <!-- Equipment -->
      @if (brew()!.equipmentUsages.length > 0) {
      <brew-card title="Equipment Used" class="detail-card">
        <div class="tag-list">
          @for (usage of brew()!.equipmentUsages; track usage.equipmentId) {
          <span class="tag">{{ getEquipmentName(usage.equipmentId) }}</span>
          }
        </div>
      </brew-card>
      }
...
export class BrewLogDetail implements OnInit {
  ...
  getEquipmentName(id: string): string {
    return this.equipment().find((e) => e.id === id)?.name || 'Unknown';
  }
