...
@Component({
  ...
  template: `
    <div class="page">
      ...
      <form (ngSubmit)="onSubmit()" class="form-layout">
        ...
        <brew-card
          title="Custom Fields"
          subtitle="Define settings specific to this equipment"
          class="form-section"
        >
          <p class="hint">
            For grinders: add grind settings. For machines: add pressure, temp
            settings, etc.
          </p>
          ...
          <div class="custom-field-row">
            ...
            <button type="button" class="remove-field" (click)="removeField(i)">
              ×
            </button>
          </div>
          }
        ...
        <div class="form-actions">
          ...
          <brew-button
            ...
            [loading]="saving()"
            [disabled]="!isValid()"
            >{{ isEdit ? 'Save' : 'Add Equipment' }}</brew-button
          >
        </div>
      </form>
    </div>
  ...
  styles: `
    ...
    .page-header { margin-bottom: var(--space-6); .back-btn { display: inline-flex; align-items: center; gap: var(--space-2); color: var(--text-tertiary); font-size: var(--text-sm); font-weight: var(--weight-medium); margin-bottom: var(--space-3); &:hover { color: var(--text-primary); } } h1 { font-family: var(--font-display); font-size: var(--text-3xl); margin: 0; } }
    .form-layout { display: flex; flex-direction: column; gap: var(--space-5); }
    .form-section { margin-bottom: 0; }
    .form-grid { display: grid; grid-template-columns: 1fr 1fr; gap: var(--space-4); margin-top: var(--space-4); }
    ...
    try {
      const fields = this.customFields
        ...
        .map((f) => ({
          ...f,
          options:
            f.type === 'select' && f.optionsString
              ? f.optionsString.split(',').map((s) => s.trim())
              : undefined,
        }));
      ...
      this.toastService.success(
        this.isEdit ? 'Equipment updated!' : 'Equipment added!'
      );
